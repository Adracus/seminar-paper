...
While it is not possible to identify a the next prime larger than $S$, there is
...
\textbf{explain number to text conversion & pseudo code}

One of the challenges of secretly sharing strings however, is to support the
widest range of characters to use. ASCII limits the characters to use very
much, since ASCII was created to display only the english language. An
encoding, which supports a large set of characters and is widely used is UTF-8.

Since UTF-8 has a very large set of characters simply converting all characters
as explained is impractical. To allow the usage of UTF-8 anyways the easiest
solution is to encode the UTF-8 strings as Base64, then apply the text to
number conversion as explained usind the ASCII charset. This is possible, since
the possible characters of Base64 are a subset of ASCII.
