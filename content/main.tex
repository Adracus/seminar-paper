...
are still safe even if one secret is unveiled or a private key
is made public. Also, duse is a product which has a strong focus
on software craftsmanship: duse is a high level implementation which
...
one secret is unveiled. The duse system also takes care of the safe
...
the underlying substitution. This was, when cryptanalysis comes in: If one looks
...
$x$ which is congruent with all $a$s with the integers $n$:
...
Adleman. As many other asymmetric encryption algorithms, RSA uses a one way function
...
As a result, a space efficient and for our use case optimal secret sharing
...
secrets. It can previously be divided into several parts making sure, the limit
...
euclidean algorithm additionally calculates the two numbers $s$ and $t$ so
...
The programming language in the back end of our service offers good integration
for line coverage analysis while the front end language for our HTML interface
...
behaviour of web service. It defines a set of constraints leading to several
...
\begin{itemize}
  ...
  RSA is missing critical for the implementation of duse. The front end ecosystem
